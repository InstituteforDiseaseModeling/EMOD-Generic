...
class ConfigKeys:
    ...
    DEMOGRAPHICS_NAME = "Demographics_Filenames"
    TOTAL_TIMESTEPS = "Simulation_Duration"
    START_TIME = "Start_Time"


class CampaignKeys:
    CAMPAIGN_DURATIONS = "Durations"
    CAMPAIGN_DIP = "Daily_Import_Pressures"
...
def set_random_campaign_file(durations, rates, campaign_filename="campaign.json", campaign_template_filename=
    ...
    campaign_json["Events"][0]["Event_Coordinator_Config"]["Intervention_Config"][CampaignKeys.CAMPAIGN_DURATIONS] = \
        durations
    campaign_json["Events"][0]["Event_Coordinator_Config"]["Intervention_Config"][CampaignKeys.CAMPAIGN_DIP] = rates
...
def load_emod_parameters(config_filename="config.json", debug=False):
    ...
    :returns param_obj:     dictionary with ConfigKeys.START_TIMESTEP, etc., keys (e.g.)
    """
    cdj = None
    ...
    param_obj[ConfigKeys.DEMOGRAPHICS_NAME] = cdj[ConfigKeys.DEMOGRAPHICS_NAME]
...
def load_campaign_file(campaign_filename="campaign.json", debug=False):
    ...
    :returns: campaign_obj structure, dictionary with CampaignKeys.CAMPAIGN_DURATIONS, etc., keys (e.g.)
    """
    with open(campaign_filename) as infile:
        cf = json.load(infile)
    campaign_obj = {}
    durations = cf["Events"][0]["Event_Coordinator_Config"]["Intervention_Config"][CampaignKeys.CAMPAIGN_DURATIONS]
    campaign_obj[CampaignKeys.CAMPAIGN_DURATIONS] = durations
    daily_import_pressures = cf["Events"][0]["Event_Coordinator_Config"]["Intervention_Config"][CampaignKeys.
        CAMPAIGN_DIP]
    campaign_obj[CampaignKeys.CAMPAIGN_DIP] = daily_import_pressures
...
def create_report_file(param_obj, campaign_obj, demographics_obj, report_data_obj, report_name, debug=False,):
    ...
    with open(report_name, "w") as outfile:
        total_timesteps = param_obj[ConfigKeys.TOTAL_TIMESTEPS]
        start_timestep = param_obj[ConfigKeys.START_TIME]
        initial_population = demographics_obj[KEY_INITIAL_POPULATION]
        rates = campaign_obj[CampaignKeys.CAMPAIGN_DIP]
        durations = campaign_obj[CampaignKeys.CAMPAIGN_DURATIONS]
        outfile.write(
            "# Test name: " + str(param_obj[ConfigKeys.CONFIG_NAME]) + ", Run number: " + 
            str(param_obj["Run_Number"]) +
