...
AcquisitionHeterogeneityVariance set to 0.25, 0.15 and 0 in 3 nodes.Base_Infectivity_Distribution is set to 
GAUSSIAN_DISTRIBUTION with Base_Infectivity_Gaussian_Mean = 1.2 and Base_Infectivity_Gaussian_Std_Dev = 0.2.
...
1. variance of infectiousness == Acquisition_Transmission_Correlation + (Base_Infectivity_Gaussian_Std_Dev)**2
2. mean of infectiousness == Base_Infectivity_Gaussian_Mean
...
class NonZeroAcquisitionTransmissionCorrelationTest(SFT):
    ...
    def test(self):
        ...
        base_infectiousness_mean = self.params[ConfigKeys.Base_Infectivity_Gaussian_Mean]
        base_infectiousness_std_dev = self.params[ConfigKeys.Base_Infectivity_Gaussian_Std_Dev]
        ...
        if self.params[ConfigKeys.Enable_Acquisition_Heterogeneity] != 1:
        ...
        elif self.params[ConfigKeys.Base_Infectivity_Distribution] != 'GAUSSIAN_DISTRIBUTION':
            self.success = False
            self.msg.append(f'BAD: please set {ConfigKeys.Base_Infectivity_Distribution} to GAUSSIAN_DISTRIBUTION in '
                            f'{self.config_filename}.\n')
            ...
            for node_variance, node_id in zip([node_1_variance, node_2_variance, node_3_variance],
                ...
                for t in range(1, simulation_duration):
                    ...
                    infectiousness_mean_list[node_id - 1][1].append(base_infectiousness_mean)
                    # check mean infectiousness
                    if math.fabs(base_infectiousness_mean - infectiousness_mean) / base_infectiousness_mean > 5e-2:
                        self.success = False
                        self.msg.append(
                            f'\tBAD: at t {t}, expected mean infectiousness is {base_infectiousness_mean}, '
                    ...
                    expected_variance = node_variance * (base_infectiousness_mean**2 + base_infectiousness_std_dev**2) * acquisition_transmission_correlation**2 + base_infectiousness_std_dev**2
                    if math.fabs(cov[0][0] - expected_variance) > \
                        ...
                        self.msg.append(f"\tBAD: variances of infectiousness should be about "
                                        ...
                                        f"{acquisition_transmission_correlation} and "
                                        f"{ConfigKeys.Base_Infectivity_Gaussian_Std_Dev} = "
                                        f"{base_infectiousness_std_dev}. Got {cov[0][0]}.\n")
                    else:
