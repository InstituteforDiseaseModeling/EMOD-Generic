...
if they are a slow progressor, who is latent and not pending relapse, then we check if they were to get an infection,
would they be a fast progressor (if not, it doesn't matter if they get a second infection or not, so we drop it), if they
are going to be a fast progressor, then we check if they will get the infection based on the contagion, interventions 
and susceptibility modifier (immunity). If so, their activation time gets recalculated for a fast progressor.
...
def parse_stdout_file(initial_timestep=0, stdout_filename="test.txt", debug=False):
    ...
    latent_count = 0
    initial_population = 5000 # placeholder for the 0th time step until we get actual data from std_out
...
def create_report_file(param_obj, stdout_data_obj, report_name, debug):
    with open(report_name, "w") as outfile:
        ...
        sft.plot_data(stdout_data_obj[1], title="Exogenous infections for tracked time steps",
                          category="exogenous_infections_tb_hiv")
