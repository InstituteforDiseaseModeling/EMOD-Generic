...
assuming each person is ON ART
...
def create_report_file(data):
    ...
    with open(report_name, "w") as outfile:
        ...
        if not sft.test_exponential(time_to_death_data, coinfection_mortality_rate_on_art, outfile,
        ...
        sft.plot_data(sorted(time_to_death_data), sorted(expected_data), label1="Actual", label2="Expected",
                          title="Time from Extrapulmonary On ART TBHIV to Death", xlabel="Data Points", ylabel="Days",
                          category="tbhiv_mortality_extrapulmonary_on_art", line = True, overlap=True)
