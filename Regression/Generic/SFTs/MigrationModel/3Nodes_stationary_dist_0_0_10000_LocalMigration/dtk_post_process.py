...
Do the validation for stationary distribution as follows and create the actual reports:
    For a inter-connected 3 Node settings without any transient nodes (i.e. for any Node pair (i,j), there exists a way
    to get from i to j at any simulation time in a finite time steps), it's equivalent to a continuous time
    Markov Chain model, and therefore we should be able to pre-calculated a stationary distribution which it will
    eventually converged to; the validation then is to run the simulation with the said node topology settings and
    make sure it converges to the pre-calculated distribution;
...
if the SFT passed, the output file (normally scientific_feature_report.txt) will show params for run as well
as "Success=True", as follows:
