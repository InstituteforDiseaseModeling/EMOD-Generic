...
This test is testing NewInfection and NewlySymptomatic events and IP information in SQLite3 db 
with Generic sim.
            ...
            for event_name in [ReportEventRecorder.Event.NewInfection.name,
                ...
                if not db_df_t_event.empty:
                    ...
                    sft_report_file.write(f"BAD: at time {t}, {len(db_df_t_event)} individuals "
                          f"({db_df_t_event['INDIVIDUAL'].tolist()}) is not in the ReportEventRecorder.csv "
                          f"for {event_name}.\n")
