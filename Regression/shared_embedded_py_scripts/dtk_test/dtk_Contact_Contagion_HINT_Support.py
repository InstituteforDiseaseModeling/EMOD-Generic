...
This SFT is testing the following statements with Multi Routes HINT enabled:
    ...
    "The contact route will be normalized by total population.(2.5.5 Contagion normalization) or group population
    (Dtk-Covid milestone 1) depends on config parameter Contact_Normalization"
...
config_keys = [ConfigKeys.Config_Name,
               ...
               ConfigKeys.Contact_Normalization]
...
Contact_Normalization_Options = ["GROUP", "POPULATION"]
...
def create_report_file(param_obj, property_df, property_obj, inset_chart_obj, insetchart_name, property_report_name,
    ...
    with open(report_name, "w") as outfile:
        ...
        contact_normalization = param_obj[ConfigKeys.Contact_Normalization]
        outfile.write("Config_name = {}\n".format(config_name))
        outfile.write("{0} = {1} {2} = {3} {4} = {5}\n".format(
            ConfigKeys.Base_Infectivity_Constant, base_infectivity,
            ConfigKeys.Run_Number, param_obj[ConfigKeys.Run_Number],
            ConfigKeys.Contact_Normalization, contact_normalization))
        ...
        for group in groups:
            ...
            for t in range(duration - 1):
                ...
                normalization_population = population.iloc[t][0] if \
                    contact_normalization == Contact_Normalization_Options[0] else \
