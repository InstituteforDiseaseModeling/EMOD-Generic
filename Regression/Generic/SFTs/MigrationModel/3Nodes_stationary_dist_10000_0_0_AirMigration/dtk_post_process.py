...
Do the validation for stationary distribution as follows and create the actual reports:
    For a inter-connected 3 Node settings without any transient nodes (i.e. for any Node pair (i,j), there exist a way
    to get from i to j at any simulation time in a finite time steps), it's equivalent to a continuous time Markov Chain
     model, and therefore we should be able to pre-calculated a stationary distribution which it will eventually
     converged to; the validation then is to run the simulation with the said node topology settings and make sure it
    converge to the pre-calculated distribution;

    Also see:
https://github.com/InstituteforDiseaseModeling/TestTeam/blob/master/MigrationTest/markov-%20chain-%20stationary-%20distribution.pdf
...
"Success=True", as follows:
	Simulation parameters: simulation_duration=1000, migration_pattern=RANDOM_WALK_DIFFUSION,
	migration_model=FIXED_RATE_MIGRATION, demog_filenames=
	:
...
"Success=False", as well as which test(s) failed, as follows:
	Simulation parameters: simulation_duration=1000, migration_pattern=RANDOM_WALK_DIFFUSION,
	migration_model=FIXED_RATE_MIGRATION, demog_filenames=[u'1x3_demographics_migration_heterogeneity.json']
	:
	Extracting data from Local migration...
		Migration bin read: {1: {2: 0.1, 3: 0.1}, 2: {1: 0.2, 3: 0.1}, 3: {1: 0.3, 2: 0.1}}
		Processing node 1.....
