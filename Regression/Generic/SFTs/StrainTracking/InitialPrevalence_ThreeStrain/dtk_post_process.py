...
def application(output_folder="output", config_filename="config.json",
    ...
    # sft.start_report_file(output_filename=report_name, config_name=cfg_object[KEY_config_name])
    initial_strain_id = None
    if cfg_object[d_sis.KEY_enable_initial_prevalence]==1 and len(cfg_object[d_sis.KEY_demographics_filenames]) == 2:
        initial_strain_dict = d_sis.load_demographic_overlay(cfg_object[d_sis.KEY_demographics_filenames][1])
        for k in initial_strain_dict:
            if initial_strain_id and k:
                raise ValueError(
                    f'Do not know what happened here. Should have had one strain, got {initial_strain_dict}')
            initial_strain_id = k
    camp_dict = d_sis.load_campaign_file(cfg_object[d_sis.KEY_campaign_filename])
    if initial_strain_id:
        camp_dict[initial_strain_id] = cfg_object[d_sis.KEY_sim_start_time]
    ...
    d_sis.create_report_file(strain_dataframe=strain_df, campaign_dictionary=camp_dict,
                       report_name=report_name, compare_messages=messages)
