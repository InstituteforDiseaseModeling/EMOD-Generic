...
This SFT is testing the HIV column in the Report_TBHIV_ByAge.csv match the last snap shot for HIV Prevalence for 
the time windows in stdout.txt.
...
def parse_output_file(output_filename="test.txt", simulation_timestep=1, debug=False):
    ...
    for line in filtered_lines:
        ...
        elif matches[1] in line: # this individual has HIV
            output_dict[time_step] += 1

    res_path = r'./DEBUG_HIV.json'
...
def application( output_folder="output", stdout_filename="test.txt", reporter_filename="Report_TBHIV_ByAge.csv",
    ...
    report_column_list = [ReportColumn.year, ReportColumn.agebin, ReportColumn.HIV]
    ...
    trs.create_report_file_prevalence(ReportColumn.HIV, ReportColumn.year, param_obj, output_dict, reporter_df,
                                     report_name, debug)
