...
class Constants:
    MAX_BASE_INFECTIVITY_CONSTANT = 2
    MAX_AMPLITUDE = 5
...
Where beta_0= Base_Infectivity_Constant, A = Infectivity_Boxcar_Forcing_Amplitude, t_start/end are
...
def set_random_config_file(config_filename="config.json", mode=Modes.LESS, debug=False):
    """
    set Base_Infectivity_Constant, Infectivity_Boxcar_Forcing_Amplitude, Infectivity_Boxcar_Forcing_End_Time and
    ...
    base_infectivity = np.random.random() * Constants.MAX_BASE_INFECTIVITY_CONSTANT
    amplitude = np.random.random() * Constants.MAX_AMPLITUDE

    config_json["parameters"][ConfigKeys.BASE_INFECTIVITY_CONSTANT] = base_infectivity
