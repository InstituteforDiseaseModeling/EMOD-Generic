...
def test_uniform(dist, p1=None, p2=None, report_file=None, round=False, significant_digits=7):
    ...
    looks_good = (p >= 5e-2)
    if not looks_good and critical_value_s:
        looks_good = (s <= critical_value_s)
    message = f"GOOD: {suffix}" if looks_good else f"BAD: {suffix}"
    if report_file is not None:
...
def test_eGaussNonNeg(dist, p1, p2, round=False, report_file=None):
    """
