...
def application(app_output_path):
    ...
    print( "Reading from path: ", app_output_path )
    with open( os.path.join( app_output_path, 'MalariaPatientReport.json' ), 'r') as f:
        j = json.load(f)
    ...
    h5f = h5py.File( os.path.join( app_output_path, 'MalariaPatientReport_hdf5_array.h5' ), 'w')
