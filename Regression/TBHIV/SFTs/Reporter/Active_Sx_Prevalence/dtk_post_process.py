...
This SFT is testing the Active_Sx column in the Report_TBHIV_ByAge.csv match the last snap shot for TB in Active Symptomatic 
prevalence for the time windows in stdout.txt.
...
matches = ["Update(): Time: ",
           ...
           "moved from Active Symptomatic to"
           ]


# can't use the common code in support library
def parse_output_file(output_filename="test.txt", simulation_timestep=1, debug=False):
    ...
    for line in filtered_lines:
        if matches[0] in line:
            ...
            output_dict[time_step] = output_dict[time_step-1]
        elif matches[1] in line: # this individual moved to Symptomatic active
            output_dict[time_step] += 1
        elif matches[2] in line: # this individual moved out from active symptomatic
            output_dict[time_step] -= 1

    res_path = r'./DEBUG_active_sx.json'
...
def application( output_folder="output", stdout_filename="test.txt", reporter_filename="Report_TBHIV_ByAge.csv",
    ...
    report_column_list =  [ReportColumn.year, ReportColumn.agebin, ReportColumn.active_sx, ReportColumn.incidence]
    ...
    trs.create_report_file_prevalence(ReportColumn.active_sx, ReportColumn.year, param_obj, output_dict, reporter_df,
                                     report_name, debug)
