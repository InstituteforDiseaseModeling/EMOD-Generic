...
class ConfigKeys:
    ...
    SUS_DIST_TYPE = "Susceptibility_Initialization_Distribution_Type"
...
def load_config_file(config_filename="config.json",
    ...
    config_object[ConfigKeys.SUS_DIST_TYPE] = config_parameters[ConfigKeys.SUS_DIST_TYPE]
...
def validate_complex_initialization_numpy(individual_dataframe, susceptibility_distribution, outfile, debug=False):
    ...
    for x in range(0, len(distribution_ages) - 1):
        ...
        # temp_df = individual_dataframe[(min_age < individual_dataframe['age']) & (max_age >
        # individual_dataframe['age'])].sort_values(by='age', ascending=True)
