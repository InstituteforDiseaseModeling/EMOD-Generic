...
def create_report_file(enabled_param, param_obj, campaign_obj, demo_obj, stdout_df, insetchart_df, report_name, debug):
    success = True
    with open(report_name, "w") as outfile:
        ...
        # for param in enabled_param:
        #     if param_obj[param] != 1:
        #         success = False
        #         outfile.write(f"{param} is disabled in this test, please make sure it's enabled in config.json. "
        #                       f"End test.\n")
        enabled_mods = []
        ...
        if set(enabled_param) != set(enabled_mods):
            success = False
            outfile.write(f"This test is expected to has these parameters enabled in config.json: {set(enabled_param)},"
                          f" please make sure they are enabled, end test.\n")
        if success:
            ...
            for t in range(len(contagion)):
                ...
                if Config.enable_infectivity_density in enabled_mods and area and halfmax : # area and halfmax could be zero
                    mod_multiplicative *= 1 - math.exp(
                        - NATURAL_LOG_OF_2 * stat_pop[t] / area / halfmax)
